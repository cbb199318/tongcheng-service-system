...
test('admin portal paper alignment smoke', async ({ page }) => {
  ...
  try {
    ...
    const dashboardShot = await takeShot(page, '03_dashboard_full');
    ...
    await waitForStable(page);
    const merchantListShot = await takeShot(page, '04_merchants_list');
    const pendingMerchantRow = page.locator('.el-table__body tr').filter({ hasText: '优选到家' }).first();
    if (await pendingMerchantRow.count()) {
      result.passes.push({
        title: '商家审核页加载正常',
        route: '#/merchants',
      ...
      result.issues.push({
        ...
        type: '演示数据问题',
        screenshot: merchantListShot
      });
    }

    await pendingMerchantRow.getByText('查看详情').click();
    await page.waitForURL(/#\/merchants\/\d+/);
    await page.waitForSelector('text=商家详情');
    await takeShot(page, '05_merchant_detail');
    result.passes.push({
      title: '商家详情页可打开',
      route: await page.evaluate(() => location.hash),
    ...
    const merchantRejectMsg = await getMessageText(page);
    const merchantRejectValidationShot = await takeShot(page, '06_merchant_reject_validation', false);
    if (merchantRejectMsg.includes('请输入驳回原因')) {
      result.passes.push({
        title: '商家驳回备注必填校验生效',
        route: await page.evaluate(() => location.hash),
        notes: `提示文案：${merchantRejectMsg}`
      });
    } else {
      result.issues.push({
        title: '商家驳回未拦截空备注',
        ...
        actual: merchantRejectMsg || '未出现必填提示',
        expected: '应阻止提交并提示填写驳回原因',
        type: '功能 bug',
        screenshot: merchantRejectValidationShot
      });
    }
    ...
    const merchantRejectFinalMsg = await getMessageText(page);
    const merchantRejectFinalShot = await takeShot(page, '07_merchant_rejected_after_submit');
    if (merchantRejectFinalMsg.includes('驳回成功')) {
      result.passes.push({
        title: '商家驳回操作可执行',
        route: await page.evaluate(() => location.hash),
        notes: `提示文案：${merchantRejectFinalMsg}`
      });
    } else {
      result.issues.push({
        title: '商家驳回操作失败',
        ...
        actual: merchantRejectFinalMsg || '未出现成功提示',
        expected: '应成功提交驳回并刷新状态',
        type: '功能 bug',
        screenshot: merchantRejectFinalShot
      });
    }
    ...
    const pendingServiceRow = page.locator('.el-table__body tr').filter({ hasText: '热水器检修' }).first();
    if (!await pendingServiceRow.count()) {
      result.issues.push({
        title: '服务审核缺少待审核演示数据',
        severity: '中',
        route: '#/services',
      ...
      await pendingServiceRow.getByText('查看详情').click();
      ...
      await page.getByRole('button', { name: '确认驳回' }).click();
      await page.waitForTimeout(600);
      const serviceRejectMsg = await getMessageText(page);
      const serviceRejectValidationShot = await takeShot(page, '10_service_reject_validation', false);
      if (serviceRejectMsg.includes('请输入驳回原因')) {
        result.passes.push({
          title: '服务驳回备注必填校验生效',
          route: await page.evaluate(() => location.hash),
          notes: `提示文案：${serviceRejectMsg}`
        });
      } else {
        result.issues.push({
          title: '服务驳回未拦截空备注',
          ...
          actual: serviceRejectMsg || '未出现必填提示',
          expected: '应阻止提交并提示填写驳回原因',
          type: '功能 bug',
          screenshot: serviceRejectValidationShot
        });
      }
      await clearMessages(page);
      ...
      await page.getByRole('button', { name: '确认驳回' }).click();
      await page.waitForTimeout(1000);
      const serviceRejectFinalMsg = await getMessageText(page);
      const serviceRejectFinalShot = await takeShot(page, '11_service_rejected_after_submit');
      if (serviceRejectFinalMsg.includes('驳回成功')) {
        result.passes.push({
          title: '服务驳回操作可执行',
          route: await page.evaluate(() => location.hash),
          notes: `提示文案：${serviceRejectFinalMsg}`
        });
      } else {
        result.issues.push({
          title: '服务驳回操作失败',
          ...
          actual: serviceRejectFinalMsg || '未出现成功提示',
          expected: '应成功提交驳回并刷新状态',
          type: '功能 bug',
          screenshot: serviceRejectFinalShot
        });
      }
    }
    ...
    await page.locator('.list-toolbar .el-select').first().click();
    await page.locator('.el-select-dropdown__item').filter({ hasText: '待接单' }).last().click();
    await waitForStable(page);
    const routeAfterStatus = await page.evaluate(() => location.hash);
    const statusRowText = await page.locator('.el-table__body tr').first().textContent();
    if (routeAfterStatus.includes('status=1') && String(statusRowText || '').includes('待接单')) {
      result.passes.push({
        title: '订单按状态筛选可用',
      ...
      result.issues.push({
        ...
        actual: `路由=${routeAfterStatus}，首行=${statusRowText}`,
    ...
    const orderDetailShot = await takeShot(page, '16_order_detail');
    ...
    await waitForStable(page);
    const userListShot = await takeShot(page, '17_users_list');
    ...
    if (!await userRow.count()) {
      ...
      if (disableMsg.includes('操作成功') && disabledText.includes('禁用') && await disabledRow.locator('text=启用').count()) {
        result.passes.push({
          title: '用户禁用操作生效',
          route: '#/users',
      ...
      if (enableMsg.includes('操作成功') && enabledText.includes('正常') && await enabledRow.locator('text=禁用').count()) {
        result.passes.push({
          title: '用户启用操作生效',
          route: '#/users',
    ...
    const firstNoticeEdit = page.locator('.el-table__body tr').first().getByText('编辑');
    if (!await firstNoticeEdit.count()) {
      result.issues.push({
        title: '公告管理无可编辑数据',
        severity: '中',
        route: '#/notices',
      ...
      await page.waitForTimeout(600);
      const noticeWarnMsg = await getMessageText(page);
      const noticeWarnShot = await takeShot(page, '21_notice_validation', false);
      if (noticeWarnMsg.includes('请完整填写公告标题和内容')) {
        result.passes.push({
          title: '公告表单必填提示合理',
          route: '#/notices',
          notes: `提示文案：${noticeWarnMsg}`
        });
      } else {
        result.issues.push({
          title: '公告表单缺少完整性校验提示',
          ...
          actual: noticeWarnMsg || '未出现必填提示',
          expected: '应提示完整填写公告标题和内容',
          type: '功能 bug',
          screenshot: noticeWarnShot
        });
      }
    ...
    await page.getByRole('button', { name: '保存' }).click();
    await page.waitForTimeout(600);
    const bannerWarnMsg = await getMessageText(page);
    const bannerWarnShot = await takeShot(page, '23_banner_validation', false);
    if (bannerWarnMsg.includes('请先上传轮播图')) {
      result.passes.push({
        title: '轮播图表单提示合理',
        route: '#/banners',
        notes: `提示文案：${bannerWarnMsg}`
      });
    } else {
      result.issues.push({
        ...
        actual: bannerWarnMsg || '未出现图片必填提示',
        expected: '应提示先上传轮播图',
        type: '功能 bug',
        screenshot: bannerWarnShot
      });
    }
    await page.getByRole('button', { name: '取消' }).click();
  } finally {
